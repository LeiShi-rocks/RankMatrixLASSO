...
for iter = 1 : maxiter
    ...
    Grad = Grad_init + Bk( Theta_new - Theta_init ); % Grad at Theta_k
    ...
    obj_line  =  smoothf(Theta_old) + lambda_run .* sum(diag(S)) + ...
        sum(sum( Grad .* (Theta_new - Theta_old) )) + ...
        (tau/2) .* sum(sum( (Theta_new - Theta_old).^2 ));
